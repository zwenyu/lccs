...
for output_dir in "${source_output_dir_list[@]}"; do
	for source_model in "${source_model_list[@]}"; do 
		for env in "${test_envs[@]}"; do
	    	python -m utils.collect_results --output_dir $output_dir --dataset $dataset --source_model $source_model --test_env $env --training_domain source
	    done
	done
done
...
for output_dir in "${lccs_output_dir_list[@]}"; do
	for source_model in "${source_model_list[@]}"; do 
		for env in "${test_envs[@]}"; do
	    	python -m utils.collect_results --output_dir $output_dir --dataset $dataset --source_model $source_model --test_env $env --training_domain target
	    done
	done
done
...
# declare -a source_output_dir_list=("output_source_models")
# declare -a lccs_output_dir_list=("output_results/LCCSLineark5n325/output_source_models_LCCSLineark5n325_resnet50_lccs_")
